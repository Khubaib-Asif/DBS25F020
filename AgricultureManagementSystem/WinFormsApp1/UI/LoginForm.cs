    ...
    public partial class LoginForm : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (loggedInUser != null)
            {
                ...
                {
                   FarmerProfileDl farmerProfiledl = new FarmerProfileDl();
                    List<FarmerProfile> famers = new List<FarmerProfile>();
                    famers=farmerProfiledl.GetAllFarmers();
                    foreach (var farmer in famers)
                    {
                        if(farmer.Email==email)
                        {
                            FarmerDashboard farmerDashboard=new FarmerDashboard(farmer);
                        ...
                        {
                            MessageBox.Show("Please First Complete Information !");
                            InfoForm info= new InfoForm (loggedInUser.Role,loggedInUser.Email);
                            info.Show();
                            this.Hide ();
                        }
                        else
                        {
                            MessageBox.Show("Please try letter");
                        }
                    }
                }
                else if (loggedInUser.Role == "Expert")
                {
                    ...
                    experts=dl.GetAllExperts();
                    foreach (var expert in experts)
                    {
                        if (expert.Email == email)
                        {
                            ExpertDashboardForm expertDashboard = new ExpertDashboardForm(expert);
                            expertDashboard.Show();
                            this.Hide();

                        }
                        else if (expert.Email ==null)
                        {
                            MessageBox.Show("Please First Complete Information !");
                            InfoForm info = new InfoForm(loggedInUser.Role, loggedInUser.Email);
                            info.Show();
